...
public class CardRestClientAdapter implements CardRepository{
    ...
    @Override
    public Card getCard( int accountNumber) {
        log.info("Servicio obtener tarjeta, buscar: [{}]" ,accountproperty.getUrl(accountproperty.getUrlGetAccount(), String.valueOf(accountNumber)));
        log.debug("este mensaje no debe aparece en el modo develop");
        CardModel response = Optional.ofNullable(restTemplate.getForObject(accountproperty.getUrl(accountproperty.getUrlGetAccount(), String.valueOf(accountNumber)),CardModel.class))
    ...
    public CardStatus UpdateStatus(CardStatus updateCard) throws URISyntaxException {
        ...
        URI uri = new URI(property.getUrl(property.getUrlBase(), property.getUrlUpdate()));
